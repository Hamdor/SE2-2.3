...
% Inhaltsverzeichnis anlegen und bei Seitenzahl 1 beginnen lassen
\setcounter{page}{1}
\tableofcontents
...
\begin{figure}
	...
	\centering\vfill\includegraphics[angle=0,scale=0.6]{imgs/Band1_FSM.png}
\end{figure}
...
Die registrierten Timer können pausiert, fortgesetzt, addiert, subtrahiert und gestoppt werden.
 \subsubsection{Diskussion der Hardware und Betriebssystem Timer}
   ...
   Wenn z.B. die BS Zeit sich während des Timers synchronisiert und dadurch um einen Augenblick erhöht, so schlägt der Realtime trozdem an dem errechneten Zeitpunkt an, auch wenn die Zeit in Wirklichkeit noch nicht verstrichen ist.
   Hingegen der Monotonic Timer lässt sich nicht von diesen Zeiten beeinflussen, er zählt seine Takte und wenn diese erreicht wurrden wird er aktiv.
