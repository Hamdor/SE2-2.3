...
\date{\today\thanks{Dokument-Version: 0.9}} % ! Hier die Versionsnummer an Versionshistorie anpassen !
\maketitle
...
\begin{small}
 \begin{longtable}{|C{2cm}|L{4cm}|C{1.5cm}|L{5.5cm}|}
  ...
  \hline
  FR-007.5 & Erkennen und Ausrichten verkehrt liegender Werkstücke & Personal & Das Personal wendet das Werkstück von Hand mit der Bohrung nach oben und legt es zurück in die Ausgangslichtschranke von Band 1\\
  \hline
  FR-007.6 & Erkennen und Ausrichten verkehrt liegender Werkstücke & Fehler & Es kommt zu einer Fehlermeldung, wenn das Werkstück nicht innerhalb der vordefinierten Zeitspanne zurück auf das Ende von Band 1 gelegt worden ist.\\
  \hline
...
\begin{figure}
    ...
    \centering\vfill\includegraphics[angle=0,scale=0.6]{imgs/Sortieranlage-Komponenten.png}
    \newline
\end{figure}
...
Pointer-to-Memberfunktion aufruft.
...
Die QNX eigenen Timer-Funktionen werden in \emph{timer\_wrapper} gewrapped, um von dem \emph{timer\_handler} 
...
Die registrierten Timer können pausiert, fortgesetzt, addiert, subtrahiert und gestoppt werden.
 \subsubsection{Diskussion der Hardware und Betriebssystem Timer}
   ...
   Die Timer werden für folgende Funktionalitäten genutzt :
   \begin{itemize}
       \item prüfen, ob die Rutsche voll ist
       \item prüfen, ob ein Werkstück hinzugefügt oder entfernt wurde; hierfür wurde ein Laufband in drei Segmente geteilt, und die Laufzeit eines Werkstück in diesen wird ausgewertet
       \item weitere?
    \end{itemize}
